...
Otherwise, it creates a new session, sets project-specific tmux environment variables, and runs `.tmux-session` if available.
...
If a selected session has `TMUX_PROJECT_ROOT` configured and the project contains an executable `.tmux-close` file, it runs that script before killing the session.
